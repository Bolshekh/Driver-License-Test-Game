...
namespace UnityEditor.Tilemaps
{
	...
	public class CustomLineBrush : GridBrush
	{
		...
		/// <summary>
		/// Which position in line will be ignored
		/// </summary>
		[Min(1)]
		public int spacingPosition;
		/// <summary>
		...
		public override void Paint(GridLayout grid, GameObject brushTarget, Vector3Int position)
		{
			if (lineStartActive)
			{
				...
				if (startPos == endPos)
				{
					if (startPos.x % spacingPosition == 0 && startPos.y % spacingPosition == 0)
				...
				else
				{
					foreach (var point in GetPointsOnLine(startPos, endPos, fillGaps))
					{
						...
						if ((paintPos.x - startPos.x) % spacingPosition == 0 && (paintPos.y - startPos.y) % spacingPosition == 0)
